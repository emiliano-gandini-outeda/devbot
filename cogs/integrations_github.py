"""
GitHub Integrations Cog - Complete rewrite with bulletproof datetime handling.
...
from utils.datetime_utils import (
    ...
    format_for_database, format_for_discord, safe_datetime_subtract, 
    safe_datetime_add, hours_ago
)
import json
import asyncio
import aiohttp
    ...
    async def initialize_repo_state(self, guild_id: str, repo_name: str):
        ...
        try:
            ...
            if not repo_data:
                ...
                # Initialize with empty state using timezone-aware datetime
                await self.bot.db.connection.execute(
...
class RepoListDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="Added", value=format_for_discord(created_at, 'R'), inline=True)
...
class RepoSettingsView(discord.ui.View):
    def __init__(self, bot, repo_name, user_id, guild_id, subscriptions):
        ...
        # Add toggle buttons
        for event_type in RepoEventTypes.all():
            enabled = subscriptions.get(event_type, False)
            self.add_item(RepoToggleButton(bot, repo_name, user_id, guild_id, event_type, enabled))
...
class RepoToggleButton(discord.ui.Button):
    def __init__(self, bot, repo_name, user_id, guild_id, event_type, enabled):
        self.bot = bot
        self.repo_name = repo_name
        self.user_id = user_id
        self.guild_id = guild_id
        ...
        emoji = RepoEventTypes.get_emoji(event_type)
        label = RepoEventTypes.get_name(event_type)
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await self.bot.db.connection.execute(
            """INSERT INTO github_subscriptions (user_id, guild_id, repo_name, event_type, enabled, created_at)
               VALUES ($1, $2, $3, $4, $5, $6)
               ON CONFLICT (user_id, guild_id, repo_name, event_type) DO UPDATE SET enabled = $5""",
            str(self.user_id), self.guild_id, self.repo_name, self.event_type, self.enabled, utc_now()
        )
        ...
        for event in RepoEventTypes.all():
            if event == self.event_type:
                subscriptions[event] = self.enabled
            else:
                # Get current status from database
                sub = await self.bot.db.connection.fetchrow(
                    """SELECT enabled FROM github_subscriptions 
                       WHERE user_id = $1 AND guild_id = $2 AND repo_name = $3 AND event_type = $4""",
                    str(self.user_id), self.guild_id, self.repo_name, event
                )
                subscriptions[event] = sub and sub['enabled'] if sub else False
        
        # Rebuild embed
        repo_data = next((repo for repo in RepoListView(self.bot, [], self.user_id, self.guild_id).repos if repo['repo_name'] == self.repo_name), None)
        if not repo_data:
            await interaction.response.send_message("Repository not found!", ephemeral=True)
            return
        ...
        state = await self.bot.db.connection.fetchrow(
            "SELECT * FROM github_repo_state WHERE guild_id = $1 AND repo_name = $2",
            self.guild_id, self.repo_name
        )
        
        stars = state['stars_count'] if state else 0
        
        embed = discord.Embed(
            title=f"🐙 {self.repo_name}",
            description=f"Repository details and notification settings",
            color=0x333333,
        ...
        # Rebuild view
        view = RepoSettingsView(self.bot, self.repo_name, self.user_id, self.guild_id, subscriptions)
