...
class Workflows(commands.Cog):
    ...
    @app_commands.describe(
        name="Workflow name",
        trigger="Trigger type (message, message:text, member_join, thread_create, channel_create)",
    ...
    @app_commands.command(name="delete-workflow", description="Delete a workflow (Admin only)")
    @app_commands.describe(workflow_name="Name of the workflow to delete")
    async def delete_workflow(self, interaction: discord.Interaction, workflow_name: str):
        if not self.bot.admin_manager.is_admin(interaction.user):
            embed = EmbedBuilder.error("Permission Denied", "Only administrators can delete workflows")
        ...
        try:
            ...
            embed = EmbedBuilder.success(
                "Workflow Deleted",
                f"Workflow **{workflow_name}** has been permanently deleted"
            )
            await interaction.followup.send(embed=embed)
            
        except Exception as e:
            embed = EmbedBuilder.error("Error", f"Failed to delete workflow: {str(e)}")
            await interaction.followup.send(embed=embed, ephemeral=True)
